...
class WizardInInvoice(models.TransientModel):
    _name = "wizard.in.invoice"
    ...
    def print_pdf(self):
        return {'type': 'ir.actions.report','report_name': 'account_move_invoice_payment.in_invoice_report','report_type':"qweb-pdf"}
    ...
    def generate_xls_report(self):
        ...
        ws1 = wb1.add_sheet(_('Past Due Accounts Payable Report'))
        fp = BytesIO()
        ...
        ws1.write_merge(row,row, 3, 7, _("Past Due Accounts Payable Report"), header_content_style)
        ...
        self.write({'state': 'get', 'report': out, 'name': _('Past Due Accounts Payable Report ')+ fecha +'.xls'})
        return {
            'type': 'ir.actions.act_window',
            'res_model': 'wizard.in.invoice',
            'view_mode': 'form',
            'view_type': 'form',
            'res_id': self.id,
            'views': [(False, 'form')],
